...
OS <- "windows"
IJscript <- "L:/Sandra/20200512_Klassen Test 1/macro.ijm"
movie_dir <- "L:/Sandra/20200512_Klassen Test 1/20200512_Klassen Test 1_movies"
target_dir <- "L:/Sandra/20200512_Klassen Test 1/20200512_Klassen Test 1_results"
mappings_file <- "L:/Sandra/20200512_Klassen Test 1/mappings.xlsx"
...
for (f in 1:length(filelist$cxd)) {
  ...
  if(file.size(filelist$cxd[f]) < 150000000) {
    next
  }
  ...
  if(file.exists(paste0(substr(filelist$cxd[f],1,gregexpr('.cxd',filelist$cxd[f])[[1]][1]),"_new_meta_data.csv"))) {
    ...
    g <- meta.data
    names(g) <- c("L1", "value")
    test <- tryCatch( g$value <- as.numeric(g$value), error=function(e){})
    ...
    if(magnification == 2 & scale_factor == 0.65) {  
  ...
  binned_by_zero <- function(x){
    ...
    position_null <- position_null %>% rowwise() %>% mutate(c=mean(c(null_starts,null_ends)))
...
findPeaksP <- function (x, thresh = 0) {
  ...
  if (!missing(thresh)) {
    if (sign(thresh) < 0) { thresh <- -thresh }
...
abdominal_segment <- function(x) {
  if(x > threshold_Xpos) { return("posterior"); } else { return("anterior"); }
}
